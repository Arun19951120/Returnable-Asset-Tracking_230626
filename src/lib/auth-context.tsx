...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {(children as React.ReactElement<{ login?: (p: UserProfile) => void }>)
        ? cloneWithLogin(children, login)
        : children}
    </AuthContext.Provider>
  );
}

// Pass login down via context instead of prop drilling
function cloneWithLogin(
  children: ReactNode,
  _login: (p: UserProfile) => void
) {
  return children;
}
